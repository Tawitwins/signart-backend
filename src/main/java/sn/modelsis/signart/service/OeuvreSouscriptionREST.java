...
public class OeuvreSouscriptionREST {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_JSON})
    public Response edit(@PathParam("id") Integer id, OeuvreSouscriptionDto dto) throws SignArtException {
        OeuvreSouscription entity = oeuvreSouscriptionConverter.dtoToEntity((dto));
        oeuvreSouscriptionFacade.edit(entity);
        OeuvreSouscriptionDto dtoRes = oeuvreSouscriptionConverter.entityToDto(entity);
        return Response.status(Response.Status.OK).entity(dtoRes).build();
    }
    @GET
